...
# Функция для анализа выгодных категорий повышенного кешбэка
def get_profitable_categories(transactions: pd.DataFrame, year: int, month: int) -> Dict[str, float]:
    ...
    for category in categories:
        ...
        profitable_categories[
            category] = cashback / total_amount * 100 if total_amount else 0  # Защита от деления на ноль

    return profitable_categories


# Функция для "Инвесткопилки"
...
# Функция для простого поиска транзакций
def get_simple_search(query: str) -> List[Dict[str, Any]]:
    """Возвращает список транзакций, содержащих запрос в описании или категории."""
    return transactions[
        (transactions['Описание'].str.contains(query)) | (transactions['Категория'].str.contains(query))].to_dict(
        'records')
