        ...
        card_data.append(
            {
                ...
                "total_spent": transactions[transactions["Номер карты"] == card][
                    "Сумма операции"
                ].sum(),
                "cashback": transactions[transactions["Номер карты"] == card][
                    "Кэшбэк"
                ].sum(),
            }
        )
...
def get_top_transactions(transactions: pd.DataFrame) -> List[Dict[str, Any]]:
    ...
    return (
        transactions.sort_values("Сумма платежа", ascending=False)
        .head(5)
        .to_dict("records")
    )
