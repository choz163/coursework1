...
def spending_by_category(transactions: pd.DataFrame, category: str, date: Optional[str] = None) -> Dict[str, float]:
    ...
    transactions_in_category = transactions_in_period[transactions_in_period['Категория'] == category]

    return transactions_in_category.groupby('Дата операции')['Сумма операции'].sum().to_frame()
...
def spending_by_weekday(transactions: pd.DataFrame, date: Optional[str] = None) -> Dict[str, float]:
    ...
    start_date = datetime.datetime.strptime(date, '%Y-%m-%d') - datetime.timedelta(days=90)
    end_date = datetime.datetime.strptime(date, '%Y-%m-%d')

    transactions_in_period = transactions[
        (transactions['Дата операции'] >= start_date) & (transactions['Дата операции'] <= end_date)]
    transactions_in_period['День недели'] = transactions_in_period['Дата операции'].dt.day_name()

    return transactions_in_period.groupby('День недели')['Сумма операции'].mean().to_frame()
...
def spending_by_workday(transactions: pd.DataFrame, date: Optional[str] = None) -> Dict[str, float]:
    ...
    return transactions_in_period.groupby('Рабочий день')['Сумма операции'].mean().to_frame()
